...
&&   $mas->is_transmit == 0 && $mas->program_mitelalefbet == 'ማታ[12:00-6:00]')
@if($i++)@endif
@endif
@endforeach
    <!doctype html>
...
<head>
    ...
    <title>ዕለቱ @if(\Illuminate\Support\Facades\Auth::user()->role_id == 10  ) {{$mas->name}}  ማታ[12:00-6:00]
        ቀን {{$mas->today_date}} @else ዕለቱ {{$ken->name}} ማታ[12:00-6:00] ቀን {{$pro->today_date}} @endif  </title>
