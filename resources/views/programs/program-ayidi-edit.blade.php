            ...
            <div class="panel-body">
                <div class="table-responsive">
                    <table id="dataTable" class="table table-bordered table-condensed table-striped">
                        ...
                        <tbody>
                        @if($i =0)@endif
                        @foreach($programmeleyaid as $pmeleya)
                            @if($i ++)@endif
                            <tr>
                                <td>{{$i}}</td>
                                ...
                                <td>
                                    <a href="{{route('program-ayidi-edit',$pmeleya->id)}}" class="btn btn-sm btn-info">አስተካክል </a>
                                </td>
                                <td>
                                    <a href="{{route('program-ayidi-delete',$pmeleya->id)}}"
                                ...
                                <td>
                                    @if($pmeleya->yeteshete == '1')
                                        @if($pmeleya->expire_contract == '0')
                                            ...
                                            <a href="{{route('program-ayidi-expire-end',$pmeleya->id)}}"
                                               class="btn btn-sm btn-danger">ውሉን አቋርጥ</a>

                                        @endif
                                        @if($pmeleya->expire_contract == '1')
                                            <a href="{{route('program-ayidi-expire-start',$pmeleya->id)}}"
