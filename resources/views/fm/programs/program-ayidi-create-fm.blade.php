...
@section('content')
    <div class="container-fluid">
        <div class="row justify-content-center">
            <div class="col-md-5">
                <div class="card">
                    <div class="card-header" style="font-size: 20px">
                        {{isset($progfm) ? 'ፕሮግራም አይዲ/መለያ/ አስተካክል ':'ፕሮግራም አይዲ/መለያ/ ጨምር'}}
                    </div>
                    <div>
                        <a href="{{route('program-ayidi-create-fm')}}" class="btn btn-info float-right">ፕሮግራም አይዲ ጨምር</a>
                    </div>
                    <form
                        action="{{isset($progfm) ? route('program-ayidi-update-fm',$progfm->id): route('program-ayidi-save-fm')}} "
                        method="post">
                        @csrf
                        @if(isset($progfm))
                            @method('PUT')
                                ...
                                <div class="row">
                                    <div class="col-md-10">
                                        <input type="text" id="name" required
                                               ...
                                               name="name"
                                               value="{{isset($progfm) ? $progfm->name : ''}}"
                            ...
                            <div class="form-group">
                                @if(isset($progfm))
                                    <div class="custom-control custom-radio custom-control-inline" style="font-size: 20px">
                                        <input type="radio" @if($progfm->yeteshete == '0') checked @endif  id="customRadioInline1" value="0" name="yeteshete" class="custom-control-input">
                                        <label class="custom-control-label" for="customRadioInline1">ያልተሸጠ </label>
                                    </div>
                                    <div class="custom-control custom-radio custom-control-inline" style="font-size: 20px">
                                        <input type="radio" @if($progfm->yeteshete == '1') checked @endif id="customRadioInline2" value="1" name="yeteshete" class="custom-control-input">
            ...
            @include('fm.programs.program-ayidi-edit-fm')
