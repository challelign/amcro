                                ...
                                <table class="table table-bordered table-striped table-responsive form-group"
                                    ...
                                    @foreach($programfm as $pro)
                                        @if($pro->program_ken_id === $ken->id
                                            ...
                                            {{--                                        <td>{{$pro->program_meleya_id}}</td>--}}
                                            <td>{{$pro->programMeleya->name}}</td>
                                            <td>{{$pro->program_file}}</td>
                                            <td>{{$pro->program_minute}}</td>
                                            <td>
                                                {{--                                            {{ date('G:i', strtotime($pro->program_mitelalefbet_seat)) }}---}}
                                                {{--                                            {{ date('G:i', strtotime($pro->program_mitelalefbet_seat2)) }}--}}
                                                {{$pro->program_mitelalefbet_seat}}
                ...
                <!-- Modal for Mastawokia -->
                    ...
                    <div class="row no-print">
                        <div class="col-md-10">
                            ...
                            @if($i > 0)
                                @if(\Illuminate\Support\Facades\Auth::user()->role_id == 5)
                                    <a href="{{route('program-list-by-date-tewat-print-fm',$ken->id)}}" target="_blank"
        ...
        $(function () {
            ...
            function sendOrderToServer() {
                ...
                $.ajax({
                    type: "POST",
                    dataType: "json",
                    url: "{{ url('fm/programs/6/program-list-by-date-tewat-fm') }}",
                    data: {
                        order: order,
                        _token: '{{csrf_token()}}'
                    },
