...
&&   $maf->is_transmit == 0 && $maf->program_mitelalefbet == 'ሌሊት[6:00-12:00]')
@if($i++)@endif
@endif
@endforeach
...
<head>
    ...
    <title>ዕለቱ @if(\Illuminate\Support\Facades\Auth::user()->role_id == 10 ) {{$maf->name}} ሌሊት[6:00-12:00]
        ቀን {{$maf->today_date}}
        @else
            {{$ken->name}} ሌሊት[6:00-12:00] ቀን {{$pro->today_date}} @endif  </title>
