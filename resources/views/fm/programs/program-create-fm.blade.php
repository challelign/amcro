...
{{--                                               value="{{$todayDate}}" --}}
                                ...
                                <div class="form-group col-md-4">
                                    <label for="title"
                                           class="col-md-12">ፕሮግራሙ ሚተላለፍበትን ሰዓት ምረጥ</label>
    ...
    <script>
        $(document).ready(function () {
            ...
            function dynamic_field(number) {
                html = '<tr>';
                html += '<td><select required name ="program_meleya_id[]"  class="form-control"><option  selected disabled value="">ፕሮግራም አይዲ ይምረጡ</option> @foreach($meleyafm as $pm ) @if($pm->expire_contract == '0') <option value="{{$pm->id}}"> {{$pm->name}}</option> @endif @endforeach </select></td>';
                ...
                html += '<td><textarea class="form-control" name="program_yizet[]" id="program_yizet" rows="3" cols="25" minlength="5" required placeholder="ይዘት  ..."></textarea></td>';
